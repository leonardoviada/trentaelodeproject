...
import it.unibo.trentalode.bot.Comment;
...
public class SelezionaCommentoDaRimuovere {
    ...
    SelezionaCommentoDaRimuovere(Stage primaryStage, News news) {
        ...
        ListView<Comment> elencoCommenti = new ListView<Comment>((ObservableList<Comment>) news.getComments().values());
        elencoCommenti.setEditable(true);
        elencoCommenti.setMaxSize(200, 150);
        root.getChildren().add(elencoCommenti);

        elencoCommenti.setCellFactory(commento -> new ListCell<Comment>() {
            public void updateItem(Comment item, boolean b) {
